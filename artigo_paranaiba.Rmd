...
FINAL_TBL_prnb_cur %>% 
  ...
  select("ASV (Sequence)","ASV header","Curated ID","Final ID (BLASTn)","BLASTn pseudo-score") %>% 
  unique() %>%  View()
...
ggsave(file = paste0(figs_path,"/",
     ...
     height = ifelse(N_IDs <= 20, 10, N_IDs/4+10),
     units = "cm",
     limitsize = FALSE,
     dpi = 300)
  ...
  pivot_wider(
    id_cols = c(
      ...
      "Primer","Primer","Read origin","Primer expected length",
                ...
                "Subfamily (BLASTn)",
                "Genus (BLASTn)",
                "max_tax","BLAST ID","BLASTn pseudo-score",
